...
public class UpdateCategoryHandler extends MenuBaseCommandHandler<UpdateCategoryCommand> {
  ...
  /**
   * if the request is to update the name and description of a category
   * then checks if there is a category with the same name exists.
   * If there is a category with the same name but the update is trying to update the description
   * then allow that
   * else throw a category already exists exception
