...
public class UpdateItemHandler extends MenuBaseCommandHandler<UpdateItemCommand> {
   ...
   * if the request is to update an item but an item with that name already exists
  ...
  List<MenuEvent> raiseApplicationEvents(Menu menu, UpdateItemCommand command) {
    return Arrays.asList(new MenuItemUpdatedEvent(command, command.getCategoryId(), command.getItemId()),
