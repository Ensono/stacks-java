...
class UpdateCategoryControllerImplTest {
  ...
  void testUpdateCategorySuccess() {
    ...
    // When
    String requestUrl = String.format(UPDATE_CATEGORY, getBaseURL(port),
            fromString(menu.getId()), fromString(category.getId()));
  ...
  void testCannotUpdateCategoryIfNoMenuExists() {
    ...
    // When
    String requestUrl =
            String.format(UPDATE_CATEGORY, getBaseURL(port), menuId, randomUUID());
    var response = this.testRestTemplate.exchange(requestUrl, HttpMethod.PUT,
  ...
  void testCannotUpdateCategoryIfNotAlreadyExists() {
    ...
    // When
    String requestUrl = String.format(UPDATE_CATEGORY,
            getBaseURL(port), fromString(menu.getId()), UUID.randomUUID());
  ...
  void testCannotUpdateCategoryNameIfNameAlreadyExists() {
    ...
    // When
    String requestUrl = String.format(UPDATE_CATEGORY, getBaseURL(port),
            fromString(menu.getId()), fromString(categoryList.get(1).getId()));
    var response = this.testRestTemplate.exchange(requestUrl, HttpMethod.PUT,
            new HttpEntity<>(request, getRequestHttpEntity()), ErrorResponse.class);
  ...
  void testUpdateCategoryWhenMultipleCategoriesExists() {
    ...
    // When
    String requestUrl = String.format(UPDATE_CATEGORY, getBaseURL(port),
            menu.getId(), categories.get(0).getId());
