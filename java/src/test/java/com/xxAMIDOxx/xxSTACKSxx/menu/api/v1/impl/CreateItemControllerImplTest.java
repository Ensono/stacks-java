...
class CreateItemControllerImplTest {
    ...
    void testNoItemNameReturnsBadRequest() {
        ...
        when(menuRepository.findById(eq(menu.getId()))).thenReturn(Optional.of(menu));
        when(menuRepository.save(any(Menu.class))).thenReturn(menu);
        CreateItemRequest request = new CreateItemRequest("", "Some Description",
            13.56d, true );

        // When
        var response = this.testRestTemplate.postForEntity(String.format("%s/v1/menu/%s/category/%s/items",
                    getBaseURL(port), menu.getId(), category.getId()), request, ErrorResponse.class);

        // Then
        then(response).isNotNull();
        then(response.getStatusCode()).isEqualTo(BAD_REQUEST);
        then(response.getBody().getDescription()).isEqualTo("Invalid Request: {name=must not be blank}");
    }
    ...
    void testNoItemDescriptionReturnsBadRequest() {
        ...
        when(menuRepository.findById(eq(menu.getId()))).thenReturn(Optional.of(menu));
        when(menuRepository.save(any(Menu.class))).thenReturn(menu);
        CreateItemRequest request = new CreateItemRequest("Some name", "",
            13.56d, true );
        ...
        then(response).isNotNull();
        then(response.getStatusCode()).isEqualTo(BAD_REQUEST);
        then(response.getBody().getDescription()).isEqualTo("Invalid Request: {description=must not be blank}");
    }
    ...
    void testInvalidPriceDescriptionReturnsBadRequest() {
        ...
        when(menuRepository.findById(eq(menu.getId()))).thenReturn(Optional.of(menu));
        when(menuRepository.save(any(Menu.class))).thenReturn(menu);
        CreateItemRequest request = new CreateItemRequest("Some name", "Item description",
            0d, true );
        ...
        then(response).isNotNull();
        then(response.getStatusCode()).isEqualTo(BAD_REQUEST);
        then(response.getBody().getDescription()).isEqualTo("Invalid Request: {price=Price must be greater than zero}");
    }
}
