...
- **Untagged-test enforcement:** The invalid-tag pipeline step now exports `MAVEN_ARGS_APPEND=-Duntagged.test.check=true`, and the `api-tests` module registers a JUnit launcher session listener (`UntaggedTestLauncherSessionListener`) that installs a shutdown hook to delete `target/failsafe-reports` and `target/site/serenity` whenever that property is present. The Azure script therefore sees empty directories when no rogue tags exist, but still fails if real reports are emitted.
  - _Applicability_: Reuse the listener/property approach in other repos that consume the shared `test-maven-post-deploy-untagged-test-check.bash` script so the guard rail works with JUnit Platform-based suites (which always emit summary files).
